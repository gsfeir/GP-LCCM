...
@summary:         Contains functions necessary for estimating Gaussian Process latent class choice models
...
from sklearn.gaussian_process.kernels import RBF, Matern, WhiteKernel, ConstantKernel, Exponentiation, ExpSineSquared, RationalQuadratic, Product, Sum
...
#from sklearn.gaussian_process.kernels import RBF, Matern, WhiteKernel, ConstantKernel, Exponentiation, ExpSineSquared, RationalQuadratic, Product
...
#kernel = WhiteKernel(noise_level=0.000464)
#kernel = Exponentiation(1.0*RBF(1.0),2)
#kernel = ExpSineSquared(length_scale=1.0,periodicity=1.0)
#kernel = RationalQuadratic(alpha=0.1, length_scale=1)
kernel1 = Matern(length_scale=1.0, nu=2.5)
kernel2 = ConstantKernel(1.0)
kernel = Sum(kernel1,kernel2)
#kernel = Product(kernel1,kernel2)
#kernel = RationalQuadratic(alpha=1.0, length_scale=1.59)
...
def initialize_parameters(X, nClasses):
    ...
    We can initialize the target classes:
        randomly
        k-means
...
#### Rnadom initializaiton
#    """Responsabilities are randomly initialized"""
#    resp = np.random.rand(n_samples, nClasses)
#    resp /= resp.sum(axis=1)[:, np.newaxis]
#    b = resp > 0.5
#    TargetClass = b.astype(int)
    TargetClass = np.random.randint(0,3,n_samples)
    return TargetClass

#### LCCM initializaiton    
###    Y_Mem = pd.read_csv(open('C:/Users/gms12/Desktop/Dropbox_Backup/00-AUB/10-Fall-2020/Gaussian Processes/Swissmetro/LCCM/All purposes/K2-Run1-1_seed0_0.8/Assignments_k2_seed0_0.8.csv','rb'))
#    all_ids = np.sort(Y_Mem["ID"].unique())
#    np.random.shuffle(all_ids)
#    split_point = int(0.67*len(all_ids))
#    estimation_ids = all_ids[:split_point]
#    prediction_ids = all_ids[split_point:]
#    estimation_Y = Y_Mem.loc[Y_Mem["ID"].isin(estimation_ids)].copy()
#    prediction_Y = Y_Mem.loc[Y_Mem["ID"].isin(prediction_ids)].copy()
    #N = len(estimation_df)
    #print("Train dataset size:", len(estimation_df))
    #print("Test dataset size:", len(prediction_df))
###    Y_estimation = Y_Mem['Class']
#    Y_prediction = prediction_Y['Class']
 
#    Y = Y_Mem['Class']
    
###    return Y_estimation    
#### Kmeans initialization   
#    kmeans = KMeans(n_clusters=nClasses).fit(X)
#    TargetClass = kmeans.labels_
...
def displayOutput(outputFile, startTime, llEstimation, llNull, lml_GP, lml_Choice, llTestNormalized, prediction_test, nClasses, 
    ...
    llEstimationChoice = sum(np.log(pChoice.T.sum(axis=1)))
    ...
    print ("Choice Log-Likelihood:".ljust(45, ' ')),(str(round(llEstimationChoice,2)).rjust(10,' ')) 
    print ("Choice Log-Likelihood:".ljust(45, ' ')),(str(round(lml_Choice,2)).rjust(10,' ')) 
...
#    resp, TargetClass = initialize_parameters(X, nClasses)
    TargetClass = initialize_parameters(X, nClasses)
    TargetClass0 = TargetClass
#    print('Class 1:',100.0*sum(TargetClass)/float(len(TargetClass)))
...
#    bb = Gqnk > 0.5
#   TargetClass2 = (bb.astype(int)).T
    TargetClass = np.argmax(Gqnk,axis=0)
#    TargetClass = Class_Prediction
    ...
    while not converged:
        ...
        print '<%s> Iteration %d: %.4f %.4f %.4f %.4f %.4f' %(currentTime, iterCounter, llNew, lml_GP, lml_Choice, llNormalized, Class1_Per)
        outputFile.write('<%s> Iteration %d: %.4f %.4f %.4f %.4f %.4f\n' %(currentTime, iterCounter, llNew, lml_GP, lml_Choice, llNormalized, Class1_Per))
        ...
        for k in range(0, nClasses):
            ...
            paramClassSpec[k] = minimize(wtLogitPanel, paramClassSpec[k], args = (expVarsClassSpec[k], altAv[k], 
                    ...
                    obsAv, choice), method = 'L-BFGS-B', jac = True, tol = llTol, options = {'gtol': grTol})['x']
